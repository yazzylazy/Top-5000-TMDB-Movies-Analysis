...
/* Test Queries */
-- Grouping Continents by film budget average
SELECT c.continentName, avg(f.budget)
FROM MovieFactTable f, Movie m, ReleaseDate d, `ProductionCountry-Movie-bridgeTable` b, ProductionCountry c
WHERE f.movie_id = m.movie_id AND f.movie_id = b.movie_id  AND b.productionCountry_id = c.productionCountry_id AND f.date_id = d.date_id
Group BY c.continentName;

-- Grouping Continents by film revenue average
SELECT c.continentName, avg(f.revenue)
FROM MovieFactTable f, Movie m, ReleaseDate d, `ProductionCountry-Movie-bridgeTable` b, ProductionCountry c
WHERE f.movie_id = m.movie_id AND f.movie_id = b.movie_id  AND b.productionCountry_id = c.productionCountry_id AND f.date_id = d.date_id
Group BY c.continentName;

/* Rollup and Drill-down queries */
-- Number of films in each country released in 2010 Q4 (Drill-down)
...
/* More complex queries to be used later */
...
-- Slice operation
SELECT m.title, m.genre, m.original_language, m.spoken_language, m.runtime
FROM MovieFactTable f, Movie m, ReleaseDate d
WHERE f.movie_id = m.movie_id AND f.date_id = d.date_id AND d.year = 2010